...
function EpicrisisMockup() {
  ...
  return (
    <div className="relative w-full max-w-md">
      ...
      <div
        aria-hidden="true"
        className="absolute -inset-6 -z-10 rounded-3xl bg-linear-to-br from-primary/10 via-secondary/10 to-secondary/10 blur-2xl"
      />
      ...
      <div className="relative overflow-hidden rounded-2xl border border-border bg-card p-6 shadow-2xl shadow-black/30 backdrop-blur">
        ...
        <div className="mb-5 flex items-center justify-between">
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full border border-success/20 bg-success/10 px-2.5 py-1">
            <span className="ambient-pulse-dot size-1.5 rounded-full bg-success" />
            <span className="text-success text-[10px] font-semibold tracking-wider uppercase">
              IA activa
            </span>
          </span>
        </div>
        ...
        <div className="space-y-2.5">
          {fields.map((field, idx) => (
            <div
              key={field.label}
              className="group rounded-xl border border-border/60 bg-card-subtle/60 p-3.5 transition-all duration-300 hover:border-primary/20 hover:bg-card-subtle"
            >
        ...
        <div className="mt-4 flex items-center gap-1.5 border-t border-border/40 pt-3">
          <Bot className="size-3.5 text-secondary/60" />
